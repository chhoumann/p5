...
However, the database design itself seemed good enough to work with despite its flaws. 
...
An example of how we approached querying with the current database design can be seen in code snippet \ref{lst:old_controller}. The code snippet shows the POST method.
...
This approach proved difficult to work with and caused more problems than benefits, especially under the aforementioned time constraints. 
Therefore, we decided to rework the design of the database completely by creating C\# models that would translate to a relational database design using EF Core migrations.
