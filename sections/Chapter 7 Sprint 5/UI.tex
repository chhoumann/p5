\section{UI}
% knox groups were given the task to create UI
% every group had to 
% committes decided it must be made in react and backend in node-js
% database UI was decided in the group that it should be a status for word count and RDF
% describe implementation briefly
% show a picture of the result
...
It decided the frontend be created in the React JavaScript library, while the backend server be created in Node.js \todo{citation needed}.
...
On the frontend, these features were implemented using some simple CSS and React JavaScript code where we use the \texttt{Fetch} API to access the backend server \todo{citation needed}. 
