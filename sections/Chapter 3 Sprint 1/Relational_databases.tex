...
This is done in a \textit{E-R diagram}. 
...
The attribute associated with the entity sets can be modelled using ovals \cite{KatjaFirstPP}, however other alternatives exists \footnotetext{Alternatives diagram styles are presented in Chapter 6.2 of \cite{DBSBook}}
...
The relationship from $book\_owner$ to $owns$ is a one-to-many relationship. That is, each book owner entity must own at least one book, and thus the relationship has full participation. 
The relationship from $book$ to $owns$ is zero-to-many. That is, the database can register books that no one owns. This relationship does not have full participation. 
Participation connections can be seen on \ref{fig:ERDiagram_Cardinality}.
