\subsection{Velocity Analysis}\label{sec:velocityAnalysis}
In sprint 3, we implemented planning poker, as described in section \ref{sec:sprintPlanningSprint3}. Since then, we held multiple sessions of planning poker during the project, as seen in e.g. section \ref{sec:sprintPlanningSprint3}.
Doing this has allowed us to measure the development of our velocity over the duration of the project. Velocity analysis is a great way to know if team productivity is actually improving, which should be a result of us implementing optimizations and removing impediments. For this reason, we will present a velocity analysis of the sprints we completed in this project, so we can document our use of agile methods to improve our velocity\cite{sutherlandScrumArtDoing2014}.
...
After most sprints, we reflected upon our process and attempted to improve it in the following sprint.
...
In general, we are very satisfied with our improvements throughout the project. Our process and approach have improved, and we learned from our experiences. More importantly, we managed to rapidly put what we learned to use, and fix impediments quickly, which visibly improved our velocity.